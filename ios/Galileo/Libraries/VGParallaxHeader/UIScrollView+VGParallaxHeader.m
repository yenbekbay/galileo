...
@property (nonatomic) NSArray *stickyViewContraints;
@property (nonatomic) NSLayoutConstraint *insetAwarePositionConstraint;
@property (nonatomic) NSLayoutConstraint *insetAwareSizeConstraint;
@property (nonatomic) UIView *containerView;
@property (nonatomic) UIView *contentView;
@property (nonatomic) VGParallaxHeaderMode mode;
@property (nonatomic, getter=isInsideTableView) BOOL insideTableView;
...
- (void)setParallaxHeaderView:(UIView *)view
    ...
    [self setParallaxHeaderView:view mode:mode height:height];
}
...
- (void)setParallaxHeaderView:(UIView *)view
    ...
    self.parallaxHeader = [[VGParallaxHeader alloc] initWithScrollView:self contentView:view
    ...
    [self addObserver:self.parallaxHeader forKeyPath:NSStringFromSelector(@selector(contentInset))
              options:NSKeyValueObservingOptionNew context:VGParallaxHeaderObserverContext];
}
...
- (void)positionTableViewParallaxHeader {
    CGFloat scaleProgress = (CGFloat)fmax(0, (1 - ((self.contentOffset.y + self.parallaxHeader.originalTopInset) / self.parallaxHeader.originalHeight)));
...
- (void)positionScrollViewParallaxHeader {
    ...
    CGFloat scaleProgress = (CGFloat)fmax(0, (height / (self.parallaxHeader.originalHeight + self.parallaxHeader.originalTopInset)));
    self.parallaxHeader.progress = scaleProgress;
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    if (self.superview && !newSuperview) {
        [self.superview removeObserver:self forKeyPath:NSStringFromSelector(@selector(contentInset))
...
- (void)addContentViewModeFillConstraints {
    ...
    self.insetAwarePositionConstraint = [self.contentView autoAlignAxis:ALAxisHorizontal toSameAxisOfView:self.containerView
                                                             withOffset:self.originalTopInset/2];
    
    NSLayoutConstraint *constraint = [self.contentView autoSetDimension:ALDimensionHeight toSize:self.originalHeight
                                                               relation:NSLayoutRelationGreaterThanOrEqual];
    constraint.priority = UILayoutPriorityRequired;
    
    self.insetAwareSizeConstraint = [self.contentView autoMatchDimension:ALDimensionHeight toDimension:ALDimensionHeight
                                                                  ofView:self.containerView withOffset:-self.originalTopInset];
...
- (void)addContentViewModeCenterConstraints {
    ...
    self.insetAwarePositionConstraint = [self.contentView autoAlignAxis:ALAxisHorizontal toSameAxisOfView:self.containerView
                                                             withOffset:(CGFloat)round(self.originalTopInset/2)];
}
...
- (void)setStickyView:(UIView *)stickyView {
    ...
    [self.containerView insertSubview:stickyView aboveSubview:self.contentView];
    // Set Local Var
...
- (void)setStickyView:(UIView *)stickyView withHeight:(CGFloat)height {
    ...
    self.stickyViewHeightConstraint = [self.stickyView autoSetDimension:ALDimensionHeight toSize:height];
}
...
- (void)updateStickyViewConstraints {
    ...
    if ([self.stickyView.superview isEqual:self.containerView]) {
        ...
        // Add Constraints
        self.stickyViewContraints = [self.stickyView autoPinEdgesToSuperviewEdgesWithInsets:UIEdgeInsetsMake(self.originalTopInset, 0, 0, 0) excludingEdge:nonStickyEdge];
    }
}
